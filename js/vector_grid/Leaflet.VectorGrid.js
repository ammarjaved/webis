...
L.VectorGrid.Protobuf = L.VectorGrid.extend({
	...
	initialize: function(url, options , buffer) {
...
// 			z: this._getZoomForUrl()	/// TODO: Maybe replicate TileLayer's maxNativeZoom
	...
	// 			console.log('Vector tile water:', json.layers.water);	// Instance of VectorTileLayer
			...
			var tileUrl = L.Util.template("test/{x}/{y}/{z}", L.extend(data, this.options));
			// var pieces = tileUrl.split("/");
			// var y1 = ((parseInt(data.z)*2)*1)-1-parseInt(data.y);
			var metaStmt = this._db.prepare("select tile_data from tiles where zoom_level = "+data.z+" AND tile_column = "+data.x+" AND tile_row = "+data["-y"]+"");
			var metaStmtCount = this._db.prepare("select count(*) as count from tiles where zoom_level = "+data.z+" AND tile_column = "+data.x+" AND tile_row = "+data["-y"]);
			while (metaStmt.step()){
                var row = metaStmt.getAsObject();
                return new Promise(function(resolve){
                	zlib.gunzip(row.tile_data, function(err, buffer) {
							var pbf = new index( buffer );
							return resolve(new VectorTile( pbf ));
            ...
            metaStmtCount.step();
            if(parseInt(metaStmtCount.getAsObject().count)==0){
            	return new Promise(function(resolve){
	...
	_readAttributesFromMbtiles: function(){
		var metaStmt = this._db.prepare("select tile_data from tiles");
		var metaStmtCount = this._db.prepare("select count(*) as count from tiles");
		metaStmtCount.step();
		var count = metaStmtCount.getAsObject().count;
		var featureIndex = 0;
		var atrbs = {};
		return new Promise(function(resolve){
			while (metaStmt.step()){
		    	...
		    	zlib.gunzip(row.tile_data, function(err, buffer) {
						...
						for (var layerName in json.layers) {
							...
							for (var i=0; i<json.layers[layerName].length; i++) {
								var feat = json.layers[layerName].feature(i);
								atrbs[layerName].push(feat.properties);
							}
						}
						if((parseInt(count)-1)==featureIndex){
							resolve(atrbs);
						}
						featureIndex++;
				});
	...
	_vectorTilesToGeoJson: function(zoomLevel){
		var metaStmt = this._db.prepare("select tile_data from tiles where zoom_level = "+zoomLevel);
		var metaStmtCount = this._db.prepare("select count(*) as count from tiles where zoom_level = "+zoomLevel);
		metaStmtCount.step();
		var count = metaStmtCount.getAsObject().count;
		var featureIndex = 0;
		var geoJson = {};
		...
		return new Promise(function(resolve){
			while (metaStmt.step()){
		    	...
		    	zlib.gunzip(row.tile_data, function(err, buffer) {
						...
						for (var layerName in json.layers) {
							for (var i=0; i<json.layers[layerName].length; i++) {
								geoJson["features"].push(json.layers[layerName].feature(i));
							}
						}
						if((parseInt(count)-1)==featureIndex){
							resolve(geoJson);
...
L.vectorGrid.protobuf = function (url, options , buffer) {
	return new L.VectorGrid.Protobuf(url, options , buffer);
};
